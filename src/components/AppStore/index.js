...
class AppStore extends Component {
  state = {searchInput: '', categorys: tabsList[0].tabId}

  CategoryUpdated = tabId => {
    this.setState({categorys: tabId})
  }

  FilterList = () => {
    const {categorys} = this.state
    const newlist = appsList.filter(each => each.category === categorys)
    return newlist
  }
  ...
  render() {
    const {searchInput, categorys} = this.state
    const updatelist = this.FilterList()

    const filterlist = updatelist.filter(each =>
